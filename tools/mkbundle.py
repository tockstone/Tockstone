...
class PebbleBundle(object):
    ...
    def add_resources(self, resources_path, resources_timestamp, sdk_version=None):
        ...
        if (
            ...
            and sdk_version["major"] >= 5
            and sdk_version["minor"] > 19
        ):
            timeline_resource_path = os.path.join(
                "build", self.subfolder, "layouts.json"
            )
            ...
            if os.path.exists(timeline_resource_path):
                ...
                self.bundle_manifest["app_layouts"] = os.path.basename(
                    timeline_resource_path
                )
    ...
    def add_jstooling(self, jstooling_path, bytecode_version):
        ...
        self.bundle_manifest["js_tooling"] = {"bytecode_version": bytecode_version}
...
def make_watchapp_bundle(timestamp, appinfo, binaries, js, outfile=None, verbose=False):
    ...
    for binary in binaries:
        ...
        if binary["watchapp"]:
            ...
            platform_bundle.add_watchapp(
                watchapp_path, timestamp, binary["sdk_version"]
            )
            bundle.has_watchapp = True

        if binary["worker_bin"]:
            worker_bin_path = os.path.expanduser(binary["worker_bin"])
            platform_bundle.add_worker(
                worker_bin_path, timestamp, binary["sdk_version"]
            )

        if binary["resources"]:
            resources_path = os.path.expanduser(binary["resources"])
            platform_bundle.add_resources(
                resources_path, timestamp, binary["sdk_version"]
            )
...
def cmd_watchapp(args):
    args.sdk_verison = dict(
        list(zip(["major", "minor"], [int(x) for x in args.sdk_version.split(".")]))
    )
...
if __name__ == "__main__":
    ...
    firmware_parser = subparsers.add_parser(
        "firmware", help="create a Pebble firmware bundle"
    )
    firmware_parser.add_argument("--firmware", help="path to the firmware .bin")
    firmware_parser.add_argument(
        "--firmware-timestamp", help="the (git) timestamp of the firmware", type=int
    )
    firmware_parser.add_argument(
        "--firmware-type",
        help="the type of firmware included in the bundle",
    ...
    watchapp_parser = subparsers.add_parser(
        "watchapp", help="create Pebble watchapp bundle"
    )
    ...
    watchapp_parser.add_argument(
        "--watchapp-timestamp", help="the (git) timestamp of the app", type=int
    )
    watchapp_parser.add_argument(
        "--javascript",
        help="path to the directory with the javascript app files to include",
    )
    watchapp_parser.add_argument(
        "--sdk-version",
        help="the SDK platform version required to run the app",
        type=str,
    )
    watchapp_parser.add_argument(
        "--resources", help="path to the generated resource pack"
    )
    watchapp_parser.add_argument(
        "--resources-timestamp",
        help="the (git) timestamp of the resource pack",
        type=int,
    )
    watchapp_parser.add_argument(
